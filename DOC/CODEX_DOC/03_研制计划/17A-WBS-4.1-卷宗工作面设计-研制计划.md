...
- 重点保证卷宗浏览、字段核对、卷间引用跳转和上下文检查能力稳定落位，并把卷宗确立为当前阶段的优先执行分支。
...
- 将 `WBS 4.1.2 / 4.1.3 / 4.1.4` 改为 `BaseLab + 模拟数据` 的独立验证链路。
- 在独立验证成立后，通过 `WBS 4.1.5` 把卷宗工作面融合进主工作台。

## 实施方式

- 按“要素设计 -> 能力验证 -> 功能验证 -> 样式验证 -> 主工作台融合”的顺序推进。
- `WBS 4.1.2 / 4.1.3 / 4.1.4` 先在 `BaseLab` 独立页面中使用模拟数据完成验证。
- `WBS 4.1.5` 只承接已在 `BaseLab` 验证通过的卷宗工作面，不再重复承担前置验证职责。
...
- [21A-WBS-4.1.5-卷宗主工作台融合-研制计划.md](/home/wgw/CodexProject/NovelStoryManager/DOC/CODEX_DOC/03_研制计划/21A-WBS-4.1.5-卷宗主工作台融合-研制计划.md)
...
- 人工核验本节点已拆成五个具体子节点，且能看出“设计 / 能力 / 功能 / 样式 / 融合”的分工。
...
- 人工核验 `WBS 4.1.2 / 4.1.3 / 4.1.4` 已明确写出 `BaseLab + 模拟数据` 的独立验证路径。
